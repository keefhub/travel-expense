...
components/
  ...
  JoinedTripSummary.tsx # (018) shared-trip summary view — takes `{ joinedTrip: JoinedTrip, storageWarning?: string | null }` and renders destination/dates/"Joined as {name}." plus an optional role="status" storageWarning and a "Back to home" link; extracted verbatim out of components/JoinTrip.tsx's joined-view branch so a second screen (the /trips/[token] viewer) can render identical markup; mounted by components/JoinTrip.tsx
