...
app/
  ...
  settings/page.tsx      # exchange-rate-settings route (006, complete) and reset-app-data route (013, complete) — 'use client'; useSyncExternalStore over a per-instance store (createTripStore via useState), mirroring app/trip/edit/page.tsx's hydration-safe pattern: renders null until determined, redirects to "/" via router.replace in a useEffect when no trip is saved, else mounts components/ExchangeRateForm.tsx plus a "Reset app data" trigger button; a `confirmingReset` boolean toggles between the normal page and components/ResetAppDataConfirm.tsx, whose onConfirm calls resetAppData() — success navigates home via router.push("/"), failure shows a role="alert" error and stays on the confirm panel — while onCancel just flips confirmingReset back off with no storage write
