...
prisma/schema.prisma  # Trip model (016: id, destinationCountry, currency, startDate, endDate,
                  ...
                  #   paymentMethod, location, description?, payerParticipantId? + payer relation
                  ...
                  #   + participant relation onDelete: SetNull, name snapshot, amount) — the
                  ...
                  #   by name on any expense they were attributed to; datasource has no `url`
...
components/
  ...
  JoinedTripSummary.tsx # (018) shared-trip summary view — takes `{ joinedTrip: JoinedTrip, storageWarning?: string | null }` and renders destination/dates/"Joined as {name}." plus an optional role="status" storageWarning, a "Back to home" link, and — since 019 — a "View participants" link to /trips/{shareToken}/participants, the participant's entry point into the participants screen; extracted verbatim out of components/JoinTrip.tsx's joined-view branch so a second screen (the /trips/[token] viewer) can render identical markup; mounted by components/JoinTrip.tsx and by components/SharedTripView.tsx (the /trips/[token] viewer this line used to leave out)
