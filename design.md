...
:root {
  ...
  --accent: #0d9488; /* teal-600 — primary actions, active nav state, links */
  --accent-foreground: #ffffff;
  --danger: #dc2626; /* red-600 — errors, over-budget */
  --warning: #d97706; /* amber-600 — out-of-range date, missing exchange rate */
  --success: #16a34a; /* green-600 — under-budget, saved confirmations */
}
...
- **Buttons:** one primary action per screen (`bg-[var(--accent)] text-[var(--accent-foreground)]`),
  secondary actions as plain text or outline. `active:scale-[0.98]` for tactile feedback on tap.
  Verify text-on-background contrast before shipping any new button (WCAG AA, 4.5:1) — no
  white-on-white or borderless ghost buttons on a matching background.
- **Loading:** a skeleton shaped like the real content (e.g. gray bars where the dashboard's totals
  will render), not a generic spinner, for anything that reads from `localStorage` after mount.
...
- `BottomNav.tsx` now uses the `--border`/`--surface`/`--accent`/`--muted` tokens and marks the
  active item with `aria-current="page"` + `--accent`.
